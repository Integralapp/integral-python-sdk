# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

import pydantic

from ....core.datetime_utils import serialize_datetime


class RequestType(pydantic.BaseModel):
    id: str
    created_at: str = pydantic.Field(alias="createdAt")
    updated_at: str = pydantic.Field(alias="updatedAt")
    deleted_at: typing.Optional[str] = pydantic.Field(alias="deletedAt")
    user_api_key: typing.Dict[str, typing.Any] = pydantic.Field(alias="userApiKey")
    route: str
    method: str
    headers: typing.Dict[str, typing.Any]
    traceback: typing.Optional[typing.Dict[str, typing.Any]]
    ip: str
    is_sandbox: bool = pydantic.Field(alias="isSandbox")
    metadata: typing.Optional[typing.Dict[str, typing.Any]]
    response_status_code: int = pydantic.Field(alias="responseStatusCode")
    response_body: typing.Dict[str, typing.Any] = pydantic.Field(alias="responseBody")

    def json(self, **kwargs: typing.Any) -> str:
        kwargs_with_defaults: typing.Any = {"by_alias": True, "exclude_unset": True, **kwargs}
        return super().json(**kwargs_with_defaults)

    def dict(self, **kwargs: typing.Any) -> typing.Dict[str, typing.Any]:
        kwargs_with_defaults: typing.Any = {"by_alias": True, "exclude_unset": True, **kwargs}
        return super().dict(**kwargs_with_defaults)

    class Config:
        frozen = True
        allow_population_by_field_name = True
        json_encoders = {dt.datetime: serialize_datetime}
